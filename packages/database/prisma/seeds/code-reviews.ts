import { PrismaClient, CodeReviewStatus, CodeReviewType } from "@prisma/client";
  ...
  const codeReviews = [
    {
      ...
      status: CodeReviewStatus.IN_PROGRESS,
      type: CodeReviewType.FEATURE,
      projectId: "proj_1",
      taskId: "task_2",
      authorId: "user_3",
      reviewerId: "user_1",
  ...
  for (const review of codeReviews) {
    await prisma.codeReview.create({
      data: {
        ...
        startedAt: new Date(),
        completedAt:
          review.status === CodeReviewStatus.IN_PROGRESS ? null : new Date(),
        metadata: {
          reviewType: "standard",
          priority: "high",
